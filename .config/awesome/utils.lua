...
get_source_volume = function(callback)
    get_default_source(function(default_source)
        awful.spawn.easy_async("pactl get-source-volume " .. default_source,
            function(current_default_source_volume_output)
...
local get_time_equation_minutes = function(fractional_year)
    return 229.18 *
    (0.000075 + 0.001868 * math.cos(fractional_year) - 0.032077 * math.sin(fractional_year) - 0.014615 * math.cos(2 * fractional_year) - 0.040849 * math.sin(2 * fractional_year))
end

local get_declination_angle = function(fractional_year)
    return 0.006918 - 0.399912 * math.cos(fractional_year) + 0.070257 * math.sin(fractional_year) -
...
local get_hour_angle = function(hour, latitude, to_sunrise)
    ...
    return hour_angle_sign *
    math.acos(math.cos(math.rad(90.833)) / (math.cos(math.rad(latitude)) * math.cos(declination_angle)) -
    math.tan(math.rad(latitude)) * math.tan(declination_angle))
end
...
reset_redshift = function()
    awful.spawn.easy_async("redshift -x", function()
    end)
end

restart_redshift = function()
    awful.spawn.easy_async("systemctl restart redshift --user", function()
    end)
end

stop_redshift = function()
    awful.spawn.easy_async("systemctl stop redshift --user", function()
    end)
end
